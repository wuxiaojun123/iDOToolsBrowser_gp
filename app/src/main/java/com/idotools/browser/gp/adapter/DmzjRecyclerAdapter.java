...
import com.idotools.browser.gp.adapter.viewHolder.DmzjViewHolderType1;
import com.idotools.browser.gp.adapter.viewHolder.DmzjViewHolderType2;
import com.idotools.browser.gp.adapter.viewHolder.DmzjViewHolderType3;
import com.idotools.browser.gp.adapter.viewHolder.DmzjViewHolderType4;
import com.idotools.browser.gp.adapter.viewHolder.DmzjViewHolderTypeAdmob;
import com.idotools.browser.gp.adapter.viewHolder.FooterViewHolder;
import com.idotools.browser.gp.adapter.viewHolder.Header2ViewHolder;
import com.idotools.browser.gp.adapter.viewHolder.HeaderViewHolder;
import com.idotools.browser.gp.bean.BannerResp;
import com.idotools.browser.gp.bean.DmzjBean;
import com.idotools.browser.gp.manager.viewpager.ViewPagerManager;
import com.idotools.browser.gp.minterface.OnItemClickListener;
...
public class DmzjRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (isHeader(viewType)) {
            ...
            return new DmzjViewHolderType1(LayoutInflater.from(mContext).inflate(R.layout.item_dmzj_type1, null));
        } else if (VIEW_TYPE_2 == viewType) {
            return new DmzjViewHolderType2(LayoutInflater.from(mContext).inflate(R.layout.item_dmzj_type2, null));
        } else if (VIEW_TYPE_3 == viewType) {
            return new DmzjViewHolderType3(LayoutInflater.from(mContext).inflate(R.layout.item_dmzj_type3, null));
        } else if (VIEW_TYPE_4 == viewType) {
            return new DmzjViewHolderType4(LayoutInflater.from(mContext).inflate(R.layout.item_dmzj_type4, null));
        } else if (VIEW_TYPE_ADMOB == viewType) {
            return new DmzjViewHolderTypeAdmob(LayoutInflater.from(mContext).inflate(R.layout.item_dmzj_admob, null));
        }
        return new FooterViewHolder(footerView);
    }

    private HashMap<String, NativeExpressAdView> admobMap = new HashMap<>();
        ...
        if (position >= 2) {
            if (position < (getItemCount() - 1)) {//判断是否需要显示底部加载更多布局
                ...
                if (bean != null) {
                    ...
                    DmzjViewHolderTypeAdmob dmzjViewHolder = (DmzjViewHolderTypeAdmob) holder;
                    FrameLayout frameLayout = dmzjViewHolder.id_fl_content;
                    NativeExpressAdView adView = admobMap.get(currentPosition + "");
                    if (adView == null) {//实例化广告
                        adView = new NativeExpressAdView(mContext);
                        ...
                        // .addTestDevice("82EDF65D3B1E788AC8A0E06E04E8EC00")

                        adView.loadAd(request);
                        admobMap.put(currentPosition + "", adView);
                    } else {
