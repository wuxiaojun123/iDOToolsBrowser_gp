package com.idotools.browser.gp.manager.http;

import com.idotools.browser.gp.bean.BannerResp;
import com.idotools.browser.gp.bean.DmzjBean;
import com.idotools.browser.gp.minterface.APIService;
...
public class AppHttpClient {
    ...
    public void requestDmzjBeanList(String cagetory,String slug, int page, int num) {
        Retrofit mRetrofit = new Retrofit.Builder().baseUrl(Constant.PATH_WORDPRESS_BASE).addConverterFactory(GsonConverterFactory.create()).build();
        APIService apiService = mRetrofit.create(APIService.class);
        //update 表示最新，hot表示最热
